...
const MAINTENANCE_ITEMS = [
    ...
    { id: 3, issue: 'Boiler not working', prop: '45 Queens Road', unit: 'Flat 1A', time: '3d ago', priority: 'High', contractor: 'Heating Co.', status: 'progress', propertyId: 1, categoryId: 'heating', photos: [IMG.maint[2]], desc: 'No hot water or heating. Boiler showing error code E119.', reportedBy: 'tenant', tenantName: 'David Wilson', reportedAt: '3d ago', visitDate: 'Tomorrow, 10:00 AM', scheduledTime: 'Tomorrow, 10:00 AM' },
    { id: 4, issue: 'Radiator not heating', prop: '15 Victoria Ave', unit: 'Flat 2A', time: '4d ago', priority: 'Medium', contractor: 'Heating Co.', status: 'progress', propertyId: 3, categoryId: 'heating', photos: [IMG.maint[1]], desc: 'Living room radiator cold while others work. Possible air lock or valve issue.', reportedBy: 'tenant', tenantName: 'Michael Lee', reportedAt: '4d ago', visitDate: 'Mar 12, 3:00 PM', scheduledTime: 'Mar 12, 3:00 PM' },
    { id: 5, issue: 'Light flickering', prop: '15 Victoria Ave', unit: 'Flat 2A', time: '5d ago', priority: 'Low', contractor: 'Electric Fix', status: 'done', propertyId: 3, categoryId: 'electrical', photos: [IMG.maint[2]], desc: 'Living room ceiling light flickering — resolved with new fitting.', reportedBy: 'tenant', tenantName: 'Michael Lee', reportedAt: '5d ago', visitDate: 'Feb 18, 2025', scheduledTime: 'Feb 18, 2025' },
    { id: 6, issue: 'Tap replaced', prop: '45 Queens Road', unit: 'Flat 1A', time: '1w ago', priority: 'Low', contractor: 'Plumber Pro', status: 'progress', propertyId: 1, categoryId: 'plumbing', photos: [IMG.maint[0]], desc: 'Kitchen tap replaced. Invoice submitted awaiting landlord approval.', reportedBy: 'landlord', tenantName: 'David Wilson', scope: 'unit', visitDate: 'Mar 1, 2025', scheduledTime: 'Mar 1, 2025', contractorRatings: { landlord: { stars: 5, comment: 'Quick turnaround and tidy finish.', at: 'Mar 1, 2025', by: 'John Smith' } } },
    { id: 7, issue: 'Hallway pipe leaking', prop: '12 Park Lane', unit: 'Communal', scope: 'communal', communalArea: 'Hallway', time: '6h ago', priority: 'Medium', contractor: 'Plumber Pro', status: 'progress', propertyId: 0, categoryId: 'plumbing', photos: [IMG.maint[1]], desc: 'Main entrance hallway pipe leaking onto floor. Landlord reports water dripping near entrance.', reportedBy: 'landlord', visitDate: 'Tomorrow, 11:30 AM', scheduledTime: 'Tomorrow, 11:30 AM' },
...
function screenRenewCompliance() {
    ...
    <div class="screen-content screen-content-sm screen-enter space-y-4">
        ...
        <div class="card p-4 rounded-2xl bg-white border border-[#E2E8F0] shadow-sm flex items-center gap-3.5 text-left">
            ...
            <div class="min-w-0 flex-1">
                <p class="text-[14px] font-bold text-[#0F172A] m-0 leading-tight">${escapeHtml(item[1])}</p>
                <p class="text-[12px] text-[#64748B] m-0 mt-1">${escapeHtml(p.name)}${escapeHtml(unitLabel)} · Current Expiry: ${escapeHtml(cert.expiryDate || item[2] || 'Not set')}</p>
            </div>
        </div>
        ...
        <div class="card p-4 rounded-2xl bg-white border border-[#E2E8F0] shadow-sm space-y-3.5 text-left">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="form-group">
                    ...
                    <input type="date" data-field="issueDate" class="form-input" value="${cert.issueDate || ''}">
                </div>
                <div class="form-group">
                    <label class="form-label">Expiry Date <span class="form-required">*</span></label>
                    <input type="date" data-field="expiryDate" class="form-input" value="${cert.expiryDate || ''}">
                </div>
            </div>

            <div class="form-group">
                <label class="form-label">Issued By / Engineer Company</label>
                <input type="text" data-field="issuedBy" class="form-input" value="${escapeHtml(cert.issuedBy || '')}" placeholder="e.g. HeatSafe Heating Ltd">
            </div>

            <div class="form-group">
                <label class="form-label">Upload Certificate PDF / Photo</label>
                <button type="button" data-action="pick-add-document-file" class="doc-upload-zone doc-upload-zone--modal w-full">
                    <i data-lucide="upload" class="w-5 h-5 text-[#2563EB]"></i>
                    <span class="doc-upload-label font-bold text-[#0F172A]">Choose file from device</span>
                    <span class="doc-upload-hint text-[#64748B]">PDF, JPG or PNG (up to 10MB)</span>
                </button>
            </div>

            <div class="form-group">
                <label class="form-label">Notes / Compliance Comments</label>
                <textarea data-field="certNotes" class="form-input h-20 resize-none" placeholder="CP12 issued for Worcester boiler...">${escapeHtml(cert.notes || '')}</textarea>
            </div>
        </div>

        <div class="p-3 rounded-xl bg-[#EFF6FF] border border-[#BFDBFE] flex items-center gap-2.5 text-left text-[12px] text-[#1E40AF]">
